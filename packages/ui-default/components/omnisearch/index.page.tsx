...
import { api, gql, i18n, request } from 'vj/utils';
...
export default new AutoloadPage('omnibar', () => {
  ...
  function SearchResult() {
    ...
    return (
      <>
        ...
        {pdocs
          .map((i) => ({ ...i, base: i.domainId !== UiContext.domainId ? `/d/${i.domainId}` : prefix }))
          .map(({ domainId, docId, pid, title, nSubmit, nAccept, base }) => (
            <a key={domainId + docId} className="omnibar-content-section omnibar-problem" href={`${base}/p/${pid || docId}`}>
              <div>
                <a
              ...
              <div>
                ...
                {pid || docId}
                <span className="icon icon-pie-chart" />
                {nAccept}/{nSubmit}
              </div>
            </a>
          ))}
          ...
          <a key={_id} className="omnibar-content-section omnibar-user" href={`${prefix}/user/${_id}`}>
            <div>
              ...
              <div className="omnibar-user-info">
                ...
                <br />
                <div>
                  <span style={{ lineHeight: '20px' }}>{uname}</span>
                </div>
              </div>
            </div>
          </a>
  ...
  $input.on(
    'input',
    debounce((ev) => search(ev.target.value), 1000),
  );
