...
export default connect(mapStateToProps)(
  class MessagePadDialogueContentContainer extends React.PureComponent {
    componentDidMount() {
        ...
        try {
          ...
          for (let i = 1; i < arr.length; i += 2) {
            if (arr[i].endsWith(':link')) {
              ...
              if (!link) continue;
              arr[i] = (
                <a style={{ color: 'wheat' }} href={link} key={i} target="_blank" rel="noreferrer">
                  {link}
                </a>
              );
            } else {
              arr[i] = (
                <span style={{ color: 'wheat' }} key={i}>
                  {data.params[arr[i]]}
                </span>
              );
            }
    ...
    renderInner() {
      ...
      const sorted = this.props.item.messages.sort((msg1, msg2) => mongoId(msg1._id).timestamp - mongoId(msg2._id).timestamp);
