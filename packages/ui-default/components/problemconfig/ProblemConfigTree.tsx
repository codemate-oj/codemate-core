import { Button, Classes, Icon, Popover, TreeNode } from '@blueprintjs/core';
...
export function SubtaskNode(props: { subtaskId: number }) {
  ...
  return (
    <li className={`${Classes.TREE_NODE} ${Classes.TREE_NODE_EXPANDED}`}>
      {subtaskId !== -1 && (
        <div className={Classes.TREE_NODE_CONTENT} onClick={() => setExpand((e) => !e)}>
              ...
              content={
                <div style={{ padding: 20 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 15 }}>
                    <Button intent="danger" onClick={deleteSubtask}>
                      {i18n('Delete')}
                    </Button>
                  </div>
                </div>
        ...
        {!cases.length && (
          <li className={Classes.TREE_NODE}>
            <div className={Classes.TREE_NODE_CONTENT}>
              ...
              <span className={`${Classes.TREE_NODE_LABEL} text-gray`}>
  ...
  return (
    <div className={Classes.TREE}>
      <ul className={`${Classes.TREE_NODE_LIST} ${Classes.TREE_ROOT}`}>
        ...
        <GlobalSettings />
        {ids.map((id) => (
          <SubtaskNode key={id} subtaskId={id} />
        ))}
        <li className={Classes.TREE_NODE} onClick={() => dispatch({ type: 'problemconfig/addSubtask' })}>
          <div className={`${Classes.TREE_NODE_CONTENT} bp5-tree-node-content-0`}>
