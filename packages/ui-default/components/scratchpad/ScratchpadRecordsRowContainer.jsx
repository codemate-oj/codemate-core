...
import { emulateAnchorClick, i18n, mongoId, substitute } from 'vj/utils';
...
const getRecordDetail = (data) => {
  if (!shouldShowDetail(data)) {
    return <span className={`record-status--text ${recordEnum.STATUS_CODES[data.status]}`}>{recordEnum.STATUS_TEXTS[data.status]}</span>;
  }
  const stat = _.pick(_.groupBy(data.testCases || [], 'status'), _.keys(recordEnum.STATUS_SCRATCHPAD_SHORT_TEXTS));
...
export default connect(
  ...
  class ScratchpadRecordsRowContainer extends React.PureComponent {
    handleRowClick(ev, id) {
      const url = substitute(decodeURIComponent(UiContext.getRecordDetailUrl), { rid: id });
    ...
    render() {
      ...
      return data.contest?.toString() === '000000000000000000000000' ? null : (
        <tr onClick={(ev) => this.handleRowClick(ev, data._id)}>
          ...
          <td className="col--memory">{shouldShowDetail(data) ? `${Math.ceil(data.memory / 1000)} MB` : '-'}</td>
          <td className="col--time">{shouldShowDetail(data) ? `${(data.time / 1000).toFixed(1)}s` : '-'}</td>
          <td className="col--at">
