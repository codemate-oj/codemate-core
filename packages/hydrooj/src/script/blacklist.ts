...
async function _address(ip: string, bset: Set<string>, uset: Set<number>, dset: Set<ObjectId>, dryrun: boolean, report: Function) {
...
async function _user(uid: number, bset: Set<string>, uset: Set<number>, dset: Set<ObjectId>, dryrun: boolean, report: Function) {
    ...
    const ddocs = await db.collection('document').find({ docType: document.TYPE_DISCUSSION, owner: uid }).sort({ domainId: 1, docId: 1 }).toArray();
    const tasks = [];
...
export const apply = (ctx) =>
    ctx.addScript(
        ...
        async ({ address = null, discuss = null, user = null, dryrun = true }, report) => {
            if (address) await _address(address, new Set(), new Set(), new Set(), dryrun, report);
            if (discuss) {
                await _discussion(discuss.domainId, new ObjectId(discuss.did), new Set(), new Set(), new Set(), dryrun, report);
            }
