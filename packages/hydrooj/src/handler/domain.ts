...
import { Handler, param, post, query, requireSudo, Types } from '../service/server';
...
registerResolver('DomainGroup', 'list(uid: Int)', '[GroupInfo]', (args, ctx) => GroupModel.list(ctx.parent._id, args.uid));
registerResolver(
    'DomainGroup',
    'update(name: String!, uids: [Int]!)',
    'Boolean',
...
class DomainUserHandler extends ManageHandler {
    ...
    async get({ domainId }) {
        ...
        const [dudocs, roles] = await Promise.all([
            domain
                .getMultiUserInDomain(domainId, {
                    $and: [{ role: { $nin: ['default', 'guest'] } }, { role: { $ne: null } }],
                })
                .toArray(),
    ...
    async postSetUser(domainId: string, uid: number, role: string) {
        ...
        await Promise.all([domain.setUserRole(domainId, uid, role), oplog.log(this, 'domain.setRole', { uid, role })]);
        this.back();
    }
...
class DomainPermissionHandler extends ManageHandler {
    ...
    async post({ domainId }) {
        ...
        for (const role in this.request.body) {
            const perms = this.request.body[role] instanceof Array ? this.request.body[role] : [this.request.body[role]];
            roles[role] = 0n;
...
class DomainUserGroupHandler extends ManageHandler {
    ...
    async postUpdate(domainId: string, name: string, uids: number[], parent?: ObjectId) {
        await GroupModel.insertOrUpdate(domainId, name, uids, parent);
        this.back();
    }
}
...
class DomainJoinHandler extends Handler {
    ...
    async post(domainId: string, code: string) {
        ...
        await Promise.all([domain.setUserRole(this.domain._id, this.user._id, this.joinSettings.role), oplog.log(this, 'domain.join', {})]);
