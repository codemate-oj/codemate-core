...
export const PROJECTION_PUBLIC: Field[] = [...PROJECTION_LIST, 'content', 'edited', 'react', 'maintainer', 'lock'];
export const HISTORY_PROJECTION_PUBLIC: (keyof DiscussionHistoryDoc)[] = ['title', 'content', 'docId', 'uid', 'time'];
...
export async function add(
    ...
    const res = await document.add(
        ...
        payload.parentType,
        payload.parentId,
        omit(payload, ['domainId', 'content', 'owner', 'parentType', 'parentId']),
    );
...
export async function get<T extends Field>(
    domainId: string,
    did: ObjectId,
    projection: T[] = PROJECTION_PUBLIC as any,
...
export function inc(domainId: string, did: ObjectId, key: NumberKeys<DiscussionDoc>, value: number): Promise<DiscussionDoc | null> {
...
export async function del(domainId: string, did: ObjectId): Promise<void> {
    const [ddoc, drdocs] = (await Promise.all([
        ...
        document
            .getMulti(domainId, document.TYPE_DISCUSSION_REPLY, {
                parentType: document.TYPE_DISCUSSION,
                parentId: did,
            })
            .project({ _id: 1, 'reply._id': 1 })
            .toArray(),
    ])) as any;
    (await Promise.all([
...
export function getMulti(domainId: string, query: Filter<DiscussionDoc> = {}, projection = PROJECTION_LIST) {
    return document
        .getMulti(domainId, document.TYPE_DISCUSSION, query)
        .sort({ pin: -1, docId: -1 })
        .project<DiscussionDoc>(buildProjection(projection));
}

export async function addReply(domainId: string, did: ObjectId, owner: number, content: string, ip: string): Promise<ObjectId> {
    const time = new Date();
    const [drid] = await Promise.all([
        document.add(domainId, content, owner, document.TYPE_DISCUSSION_REPLY, null, document.TYPE_DISCUSSION, did, { ip, editor: owner }),
...
export async function editReply(domainId: string, drid: ObjectId, content: string, uid: number, ip: string): Promise<DiscussionReplyDoc | null> {
    await coll.insertOne({
        domainId,
        docId: drid,
        content,
...
export function getMultiReply(domainId: string, did: ObjectId) {
    return document.getMulti(domainId, document.TYPE_DISCUSSION_REPLY, { parentType: document.TYPE_DISCUSSION, parentId: did }).sort('_id', -1);
}
...
export async function addTailReply(
    ...
    const time = new Date();
    const [drdoc, subId] = await document.push(domainId, document.TYPE_DISCUSSION_REPLY, drid, 'reply', content, owner, { ip, editor: owner });
    await Promise.all([
        coll.insertOne({
            domainId,
            docId: subId,
        ...
        document.set(domainId, document.TYPE_DISCUSSION, drdoc.parentId, { updateAt: time }),
    ]);
    return [drdoc, subId];
}
...
export async function editTailReply(
    ...
    const [, drrdoc] = await Promise.all([
        ...
        document.setSub(domainId, document.TYPE_DISCUSSION_REPLY, drid, 'reply', drrid, { content, edited: true, editor: uid }),
    ]);
    return drrdoc;
}
...
export function getHistory(domainId: string, docId: ObjectId, query: Filter<DiscussionHistoryDoc> = {}, projection = HISTORY_PROJECTION_PUBLIC) {
    return coll
        .find({ domainId, docId, ...query })
        .sort({ time: -1 })
...
export function addNode(domainId: string, _id: string, category: string, args: any = {}) {
    return document.add(domainId, category, 1, document.TYPE_DISCUSSION_NODE, _id, null, null, args);
}
...
export function apply(ctx: Context) {
    ctx.on('problem/delete', async (domainId, docId) => {
        const dids = await document
            .getMulti(domainId, document.TYPE_DISCUSSION, { parentType: document.TYPE_PROBLEM, parentId: docId })
            .project({ docId: 1 })
            .map((ddoc) => ddoc.docId)
            .toArray();
        const drids = await document
            .getMulti(domainId, document.TYPE_DISCUSSION_REPLY, { parentType: document.TYPE_DISCUSSION, parentId: { $in: dids } })
            .project({ docId: 1 })
            .map((drdoc) => drdoc.docId)
            .toArray();
        return await Promise.all([
    ...
    ctx.on('problem/edit', async (result) => {
        const dids = await document
            .getMulti(result.domainId, document.TYPE_DISCUSSION, { parentType: document.TYPE_PROBLEM, parentId: result.docId })
            .project({ docId: 1 })
            .map((ddoc) => ddoc.docId)
            .toArray();
