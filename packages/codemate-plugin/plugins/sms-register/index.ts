...
import {
    logger,
    SendSMSFailedError,
    UserNotBindPhoneError,
    VerifyCodeError,
    VerifyTokenCheckNotPassedError,
} from './lib';
...
export class RequestLoginSMSCodeHandler extends Handler {
    ...
    async post(domainId: string, verifyToken: string, uname?: string, phoneNumber?: string) {
        ...
        if (uname) {
            ...
            const sendResult = await global.Hydro.lib.sms(
                `【codemate】您的验证码是${verifyCode}，600s内有效`,
                user.phoneNumber,
            );
            if (!sendResult) throw new SendSMSFailedError();
            this.response.body = {
